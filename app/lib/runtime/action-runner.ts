...
import type { ActionAlert, AstroAction, DeployAlert, FileHistory, SupabaseAction, SupabaseAlert } from '~/types/actions';
import { createScopedLogger } from '~/utils/logger';
import { unreachable } from '~/utils/unreachable';
import type { ActionCallbackData } from './message-parser';
import type { AstroShell } from '~/utils/shell';

const logger = createScopedLogger('ActionRunner');

export type ActionStatus = 'pending' | 'running' | 'complete' | 'aborted' | 'failed';

export type BaseActionState = AstroAction & {
...
export type FailedActionState = AstroAction &
...
export class ActionRunner {
  ...
  async handleSupabaseAction(action: SupabaseAction) {
    ...
    await this.#ensureSupabaseScaffold();
  ...
  async #ensureSupabaseScaffold() {
    const webcontainer = await this.#webcontainer;
    const supabaseDir = nodePath.join(webcontainer.workdir, 'supabase');
    const functionPath = nodePath.join(webcontainer.workdir, 'supabase/functions/hello/index.ts');
    const configPath = nodePath.join(webcontainer.workdir, 'supabase/config.toml');
    const migrationPath = nodePath.join(webcontainer.workdir, 'supabase/migrations/000_init.sql');
    ...
    try {
      await webcontainer.fs.mkdir(nodePath.join(supabaseDir, 'functions/hello'), { recursive: true });
      await webcontainer.fs.mkdir(nodePath.join(supabaseDir, 'migrations'), { recursive: true });
    } catch (error) {
      logger.debug('Supabase scaffold mkdir skipped/failed:', error);
    }

    if (!(await readIfExists(configPath))) {
      await this.#runFileAction({
        type: 'file',
        filePath: configPath,
        content: [
          '# Auto-generated by Astro',
          "project_id = 'replace-with-supabase-project-ref'",
          '',
          '[api]',
          'enabled = true',
          'port = 54321',
    ...
    if (!(await readIfExists(functionPath))) {
      await this.#runFileAction({
        type: 'file',
        ...
        content: [
          ...
          '',
          "serve(() => new Response(JSON.stringify({ ok: true, service: 'astro-supabase' }), {",
    ...
    if (!(await readIfExists(migrationPath))) {
      await this.#runFileAction({
        type: 'file',
        filePath: migrationPath,
        content: [
          '-- Astro baseline migration',
          '-- Add your first schema changes in subsequent migration files.',
