  ...
  return (tree: any) => {
    visit(tree, (node: any) => {
      if (node.type === 'html' && node.value && node.value.startsWith('<think>')) {
        ...
        node.value = `<div class="__AstroThought__">${cleanedContent}`;
...
const rehypeSanitizeOptions: RehypeSanitizeOptions = {
  ...
  attributes: {
    ...
    div: [
      ...
      'data*',
      ['className', '__AstroArtifact__', '__AstroThought__', '__AstroQuickAction', '__AstroSelectedElement__'],

      // ['className', '__AstroThought__']
    ],
    button: [
      ...
      'name',
      'value',
      ['className', '__AstroArtifact__', '__AstroThought__', '__AstroQuickAction'],
    ],
  },
  strip: [],
};
