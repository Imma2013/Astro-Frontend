...
export function GitHubUserProfile({ user, className = '' }: GitHubUserProfileProps) {
  return (
    <div
      className={`flex items-center gap-4 p-4 bg-Astro-elements-background-depth-1 dark:bg-Astro-elements-background-depth-1 rounded-lg ${className}`}
    >
      <img
        src={user.avatar_url}
        alt={user.login}
        className="w-12 h-12 rounded-full border-2 border-Astro-elements-item-contentAccent dark:border-Astro-elements-item-contentAccent"
      />
      <div>
